...
const columns = [
  ...
  ["Fraction Guides", [["Percent and Decimal", "/percent-and-decimal"], ["Equivalent Fractions", "/equivalent-fractions"]]],
  ["Legal", [["Privacy Policy", "/privacy"], ["Terms of Use", "/terms"], ["Contact", "/contact"]]]
] as const;

export function Footer(): React.JSX.Element {
  return <footer className="mt-16 bg-[#061f49] text-white"><div className="mx-auto grid max-w-[1240px] gap-8 px-4 py-10 sm:grid-cols-2 sm:px-6 lg:grid-cols-6 lg:px-8"><div><BrandLogo inverse compact /><p className="mt-3 max-w-xs text-sm leading-6 text-blue-100/75">Free step-by-step tools for calculus and the algebra used to learn it.</p></div>{columns.map(([title, links]) => <div key={title}><h2 className="text-sm font-bold">{title}</h2><ul className="mt-3 space-y-2">{links.map(([label, href]) => <li key={href}><Link href={href} className="text-sm text-blue-100/75 hover:text-white">{label}</Link></li>)}</ul></div>)}</div><div className="border-t border-white/10 px-4 py-5 text-center text-xs text-blue-100/60">© {new Date().getFullYear()} Calculus Solver. All rights reserved.</div></footer>;
}
